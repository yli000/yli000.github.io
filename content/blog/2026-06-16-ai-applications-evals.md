...
错例采样：100条起看，普遍存在的错例应该会出现在其中；使用一些[采样](https://hamel.dev/blog/posts/evals-faq/how-can-i-efficiently-sample-production-traces-for-review.html)方法，如聚类用户输入、关注异常值（e.g. 延迟、工具调用）、通用指标的高/低分回复、关键维度（用户类型、功能、输入类别）。

资料里还提到了很多操作细节，如谁来执行错例评估（专家/工程师/外包），先大致了解。
